...
import axios from 'axios'; // Usamos axios para manejar mejor los headers de autenticación
...
const MEMBER_API_URL = `${BASE_URL}/api/group`; // Usamos la misma base para las rutas de miembros
...
const mapApiToGroupDetail = (group) => {
    ...
    // CAMBIO: Usamos MEMBER_COUNT que viene del servicio getGroupById
    const membersTotal = group.MEMBER_COUNT ? group.MEMBER_COUNT.toLocaleString('es-ES') : '0';

    // Nota: Si el backend ya retorna la URL completa (incluyendo BASE_URL), 
    // se debe usar directamente, sin concatenar BASE_URL aquí.
    return {
        id: group.ID_GROUP, 
        name: group.GROUP_NAME_DSC,
        subtitle: group.SUBTITLE_DSC || 'Sin descripción.', 
        communityType: group.COMMUNITY_TYPE_DSC,
        membersTotal: membersTotal, // CANTIDAD DE MIEMBROS DE LA BASE DE DATOS
        isStreamer: isStreamer,
        
        bannerUri: group.BANNER_IMG_URL 
            ? group.BANNER_IMG_URL // Asumiendo que el backend ya incluyó BASE_URL
            : `https://picsum.photos/600/200?random=${group.ID_GROUP}_bg`,
   
        profilePicUri: group.PROFILE_IMG_URL 
            ? group.PROFILE_IMG_URL // Asumiendo que el backend ya incluyó BASE_URL
            : `https://picsum.photos/100/100?random=${group.ID_GROUP}_pfp`,
        
        membersOnline: isStreamer ? '3,400' : '1,234', // Este es un dato ficticio (online)
        isLive: isLive,
...
// ELIMINAMOS MEMBERS_DATA para usar la lista traída de la API
// const MEMBERS_DATA = [
...
const AvisarDirectoModal = ({ isVisible, onClose }) => {
    ...
    // ... (El código de AvisarDirectoModal se mantiene sin cambios)
    const handleAvisar = () => {
        if (isLiveNow) {
...
const GroupDetailView = ({ navigation, route }) => {
    ...
    // NUEVO ESTADO: Rol del usuario y lista de miembros
    const [userRole, setUserRole] = useState(null); // 'ADMIN', 'MODERATOR', 'MEMBER', o null
    const [membersList, setMembersList] = useState([]); // Lista de miembros de la API
    ...
    // Función unificada para obtener detalles del grupo y el rol del usuario logueado
    const fetchGroupDetailAndRole = useCallback(async () => {
        ...
        try {
            // --- 1. Fetch Group Details (and Member Count) ---
            // No requiere token si la ruta GET es pública
            ...
            // --- 2. Fetch User Role (for Settings visibility) ---
            const roleResponse = await fetch(`${MEMBER_API_URL}/${groupId}/role`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`, // ¡Esto es crucial!
                },
            });
    ...
    // Función para obtener la lista completa de miembros (se llama al cambiar a la pestaña 'Miembros')
    const fetchMembersList = useCallback(async () => {
        ...
        try {
            // Llama a /api/group/:groupId/members
            const response = await fetch(`${MEMBER_API_URL}/${groupId}/members`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`, // Requiere token
                },
            });
            ...
            // Mapea la respuesta de la API (que incluye el objeto 'user') al formato de MemberListingRow
            const mappedMembers = data.map(m => ({
                id: m.user.ID_USER, // ID de usuario
                username: m.user.USERNAME_DSC,
                role: m.MEMBER_ROLE_DSC,
                // Asumiendo que el campo PROFILE_PIC contiene la URL completa o el path que necesita arreglo
                avatarUri: m.user.PROFILE_PIC 
                    ? m.user.PROFILE_PIC.startsWith(BASE_URL) 
                        ? m.user.PROFILE_PIC 
                        : `${BASE_URL}${m.user.PROFILE_PIC.replace('src/public', '')}` // Si es un path relativo, lo convierte a URL completa
                    : `https://picsum.photos/40/40?random=${m.user.ID_USER}`, 
                isOnline: false, // Dato ficticio si no tienes un sistema de presencia
            }));
    ...
    // Llama a la lista de miembros cuando se activa la pestaña
    useEffect(() => {
        ...
        if (activeTab === 'Miembros') {
            return (
                <View style={styles.membersListContainer}>
                    ...
                    {/* CAMBIO: Usamos la cantidad total del objeto de grupo (API 1) */}
                    <Text style={styles.membersCountSubText}>{groupData.membersTotal} miembros • {groupData.membersOnline} en línea</Text>
                    {/* CAMBIO: Iteramos sobre la lista de miembros traída por la API (API 2) */}
                        ...
                        <View style={styles.groupContentRow}>
                            ...
                            <View style={styles.statsAndActions}>
                                ...
                                <View style={styles.actionButtonsRow}>
                                  ...
                                  {/* CAMBIO: Renderizado condicional de la opción de Configuración solo para ADMIN */}
                                    {userRole === 'ADMIN' && (
                                        <TouchableOpacity 
                                    ...
                                    {/* CAMBIO: Renderizado condicional de Avisar Directo solo para ADMIN */}
